...
class torch2img():
    ...
    def write(self, torch_img, inverse = True ,filename=None, ):
        ...
        if len(torch_img.shape) == 3:
            ...
            assert channel in [3,20,11,2], '[channel, x, y] need channel in [3, 20, 11] but got ' + str(torch_img.shape)
            if channel == 20:
                #gtl mode
    ...
    def write_(self, torch_img,  inverse = True, filename=None,):
        if filename is None: filename = self.savename + 'temp'
        assert len(torch_img.shape) in [2,3]
        filename = filename + '.jpg'
        if len(torch_img.shape) == 3:
        ...
        if inverse:
            img = 1-img
        save_image(img, filename)
    def vcat(self,tensors):
        n = len(tensors)
        for i in range(n):
            self.write(tensors[i], filename=self.savename+'temp%s'%i)
        npys = [cv2.imread(self.savename+'temp%s.jpg'%i) for i in range(n)]
        cv2.imwrite(self.savename+'temp.jpg',cv2.vconcat(npys))
    def vcat_(self,tensors):
        n = len(tensors)
        for i in range(n):
            self.write_(tensors[i], filename='temp%s'%i)
        npys = [cv2.imread(self.savename+'temp%s.jpg'%i) for i in range(n)]
        cv2.imwrite(self.savename+'temp.jpg',cv2.vconcat(npys))  
    ...
    def read(self,):
        import matplotlib.pyplot as plt
        img = cv2.imread(self.savename+'temp.jpg')
        plt.imshow(img)
        # plt.colorbar()
        plt.show()
    def test_cat(self,):
    ...
    def write_all(self, tensors):
        ...
        for i,tensor in enumerate(tensors):
            if i!=5:
                ...
                self.vcat_(tensor)
            imgs.append(cv2.imread(self.savename+'temp.jpg'))
    ...
    def genimg_(self, img, out, gts, gtl, filename, msg='temp', savefolder=None): #img = [batch, ch, x, y]
        ...
        cv2.imwrite(self.savename+'temp.jpg',img) # for send to line
    ...
    def write_header_msg(self, oriimg, msg):
        ...
        cv2.imwrite(self.savename+'testhead.jpg', img)
        return img
        # cv2.imwrite(self.savename+'header.jpg',img)
    def test_write_header_msg(self,):
        tempimg = cv2.imread(self.savename+'temp.jpg')
